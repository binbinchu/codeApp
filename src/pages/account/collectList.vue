  ...
  <div class="shopping-div recommend">
    <scroll-view :scroll-y="Y" @scrolltolower="loadMore" class="classify-scroll-view">
      <block v-if="!complete&&loaded||!complete&&!loaded">
        ...
        <div class="no-more" v-if="!complete&&!loaded"><i-divider color="#ed3f14" lineColor="#ed3f14" i-class="set-divider">加载已经完成,没有其他数据</i-divider></div>
      </block>
      <block v-else-if="complete&&loaded">
        <div class="nothing"><i-divider color="#ed3f14" lineColor="#ed3f14" i-class="set-divider">暂无数据</i-divider></div>
      </block>
    </scroll-view>
  </div>
</template>
